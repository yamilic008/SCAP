...
@section('content')
   <div class="box">
            <div class="box-header">
              ...
              <a href="{{ route('pdfarea') }}" class="btn btn-info pull-right" target="_blank" >Reporte</a>

              <a href="{{ route('area.create') }}" class="btn btn-success pull-right" >Nueva Area</a>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <table id="usuarios" class="table table-bordered table-striped table-hover">
               ...
               @foreach($dato as $dato)
                  <tr>
                      <td> {{ $dato->id }}</td>
                      <td> {{ $dato->nombre }}</td>
                      <td> {{ $dato->gestion->nombre}}</td>
                      <td> {{ $dato->created_at->format('d/M/Y') }}</td>
                      <td>
                       <!--  <a href="#" class="btn btn-xs btn-default"><i class="fa fa-eye"></i></a> -->
                          <a href="{{route ('area.edit',$dato->id)}}" class="btn btn-xs btn-info"><i class="fa fa-pencil"></i></a>
                          <form method='POST' 
                                action="{{route('area.destroy',$dato['id'])}}"
